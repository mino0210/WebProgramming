...
function KakaoMap({ pins, onMapClick, onPinsLoaded }) {
  ...
  useEffect(() => {
    ...
    getReports().then((res) => onPinsLoaded?.(res.data?.data || res.data || [])).catch(() => {})
    // 페이지 로드 시 자동으로 내 위치 표시
    if (navigator.geolocation) {
      ...
      <style>@keyframes ping{0%{transform:scale(0.8);opacity:1;}100%{transform:scale(2.2);opacity:0;}}</style>`
        myMarkerRef.current = new kakao.maps.CustomOverlay({
          map: mapObj.current, position: latlng, content, zIndex: 10,
        })
      })
    }
  }, [])
  ...
  return (
      <div style={{ position: 'relative', flex: 1, height: '100%' }}>
        ...
        <div style={styles.controlBox}>
          ...
          <div style={styles.divider} />
          <button style={styles.controlBtn} onClick={handleMyLocation}>🎯</button>
        </div>
      </div>
  )
}
