...
function ReportFeed({ pins = [] }) {
  ...
  const handleCardClick = (report) => {
    ...
    const selectedId = selected ? getReportId(selected) : null

    setSelected(selectedId === reportId ? null : report)
  }

  const color = selected
    ? CATEGORY_COLOR[selected.categoryName] || '#8b5cf6'
    : '#8b5cf6'
      ...
      {selected && (
        <div style={styles.detail}>
          <div style={styles.detailHeader}>
            ...
            <button style={styles.detailClose} onClick={() => setSelected(null)}>
              ✕
            </button>
          </div>

          <div style={styles.detailTitle}>{selected.title || '제목 없음'}</div>
